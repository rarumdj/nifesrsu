...
$id = $_GET['post_id'];
$q_query= "SELECT * FROM images WHERE post_id='$id'";
$q_result = mysql_query($q_query);

$m_query= "SELECT * FROM videos WHERE post_id='$id'";
$m_result = mysql_query($m_query);

$p_query= "SELECT * FROM updates WHERE post_id='$id'";
$p_result = mysql_query($p_query);

if (mysql_num_rows($q_result) > 0) {

  while ($row = mysql_fetch_assoc($q_result)) {
$content = $row['img_path'];
$file_name = $row['image'];
...
exit;
    ...
    } elseif (mysql_num_rows($p_result) > 0) {
      
      while ($row = mysql_fetch_assoc($p_result)) {
$content = $row['img_path'];
$file_name = $row['image'];
